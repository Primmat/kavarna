...
class FormBuilderProcessor extends WireData {
	...
	public function ___render($id = null) {
		...
		if($isExportD) {
			...
			if(count($errors)) $out .= $this->renderErrors($errors, true);
			...
			$this->clearErrors();
			$this->executePluginActions('renderReady');
			$formOut = $this->renderEmbedMethodD($form, $fileEmbedD, $framework, $submitType, $errors);
